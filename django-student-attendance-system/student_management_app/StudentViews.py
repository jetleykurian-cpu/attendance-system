...
def student_feedback_save(request):
    if request.method != "POST":
        ...
        try:
            if recipient_type == 'staff' and staff_id:
                staff_obj = Staffs.objects.get(id=staff_id)
                add_feedback = FeedBackStudent(student_id=student_obj, staff_id=staff_obj, feedback=feedback, feedback_reply="")
            else:
                # Send to admin
...
@csrf_exempt
def student_delete_feedback(request):
    feedback_id = request.POST.get('id')
    student_obj = Students.objects.get(admin=request.user.id)
    
    try:
        feedback = FeedBackStudent.objects.get(id=feedback_id, student_id=student_obj)
        feedback.delete()
